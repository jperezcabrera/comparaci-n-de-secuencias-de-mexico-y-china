...
alinea = function(gen,gen2){
  ...
  for (fila in seq(2,nrow(m))){
    for (col in seq(2,ncol(m))){
      #cat(fila,",",col,gen[col-1]==gen2[fila-1],"\n")
...
file2 = read.fasta("omicronmexico.fasta", forceDNAtolower = FALSE)
...
for (i in seq_along(file)){
  ...
  for (j in seq(i, length(file2), 12)){
    ...
    if (length(gen) == length(gen2)){
      ...
      if (secuencias_con_indels>10) break
      if (i!=3) next
